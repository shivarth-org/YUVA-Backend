...
const State = require("../../databases/mongodb/models/State");
...
router.post("/insert", async (req, res) => {
    try {
        ...
        const state_obj = await State.create({
            name: request.name,
            city: request.city,

        });
        if (!state_obj) {
            return res.status(404).json({ statusText: statusText.STATE_NOT_FOUND });
        }
        res.status(200).json({ statusText: statusText.STATE_CREATE_SUCCESS, data: state_obj });
    } catch (err) {
